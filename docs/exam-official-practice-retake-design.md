...
- 重作**沒有個人限時倒數**；可編輯期限固定為正式測驗活動的 `close_at + 24 小時`。
- 管理員關閉正式測驗只收正式作答，不影響重作；在重作期限前可不限次數返回、修改與自動儲存答案。
- 使用者可按「暫時完成練習」離開，但這只是離開畫面，不會鎖定答案；原活動結束後 24 小時內仍可回來修改。
- 重作期限到達後，由伺服器獨立凍結練習卷，之後只能查看、不能再修改。
...
- 從正式首考送出後開始，到 `close_at + 24 小時` 前可建立、返回及修改練習卷。
- 管理員手動關閉或伺服器在 `NOW() >= close_at` 自動關閉，只結束正式作答並開放正式評分，不鎖定重作。
- `practice_retake_enabled` 開關預設開啟；練習截止不另存可漂移欄位，統一由 `close_at + INTERVAL '1 day'` 推導。
- `close_at` 是正式活動自動結束的權威時間；`close_at + 24 小時` 是重作練習自動鎖定的權威時間。
- 正式成績可在正式測驗關閉後公布，不必等待重作期限；重作資料不參與正式評分、統計或公布條件。
